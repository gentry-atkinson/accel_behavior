...
def build_AE(train_set):
    ...
    enc = Reshape((SAMPLE_LENGTH, 1))(inp)
    enc = BatchNormalization(scale=False)(enc)
    enc = Conv1D(filters=25, kernel_size=16, activation='linear', use_bias=True)(enc)
    print(enc.shape)
    enc = MaxPooling1D(pool_size=25, data_format='channels_first')(enc)
    #enc = GlobalMaxPooling1D()(enc)

    enc = Flatten()(enc)
    hidden = Dense(35, activation='sigmoid')(enc)

    print("Building decoder......................")
    dec = hidden
    #dec = Dense(370, activation='sigmoid')(dec)
    print(dec.shape)
    dec = Reshape((35, 1))(dec)
    dec = Conv1DTranspose(dec, filters=50, strides=1, kernel_size=16,)
    print(dec.shape)
